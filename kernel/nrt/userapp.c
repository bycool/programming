#include <stdio.h>
#include <stdlib.h>
#include <sys/socket.h>
#include <string.h>
#include <linux/netlink.h>
#include <fcntl.h>
#include <sys/mman.h>

#define NETLINK_TEST 17
#define MSG_LEN 100

struct msg_to_kernel
{
    struct nlmsghdr hdr;
    char data[MSG_LEN];
};
struct u_packet_info
{
    struct nlmsghdr hdr;
    char msg[MSG_LEN];
};


void* read_relay_fs(void){
	int relay_fd = open("/mnt/debugfs/nrt0",O_RDWR,770);
	char buf[16] = {0};
	int ret = 0;
	if(relay_fd < 0){
		printf("open relay fail\n");
		return ;
	}
	while(1){
		ret = read(relay_fd, buf, 16);
		printf("%s\n", buf);
		sleep(1);
	}
	close(relay_fd);
}

int main(int argc, char* argv[]) 
{
	int cnt = 20;
    char *data = "This message is from eric's space";
    //初始化
    struct sockaddr_nl local;
    struct sockaddr_nl kpeer;
    int skfd, ret, kpeerlen = sizeof(struct sockaddr_nl);
    struct nlmsghdr *message;
    struct u_packet_info info;
    char *retval;
    message = (struct nlmsghdr *)malloc(1);

	pthread_t relay_read;
	if(pthread_create(&relay_read, NULL, read_relay_fs, NULL) == -1)
		printf("create_relay_fail\n");

	pthread_detach(relay_read);


    skfd = socket(PF_NETLINK, SOCK_RAW, NETLINK_TEST);
    if(skfd < 0){
        printf("can not create a netlink socket\n");
        return -1;
    }

    memset(&local, 0, sizeof(local));
    local.nl_family = AF_NETLINK;
    local.nl_pid = getpid();
    local.nl_groups = 0;
    if(bind(skfd, (struct sockaddr *)&local, sizeof(local)) != 0){
        printf("bind() error\n");
        return -1;
    }
    memset(&kpeer, 0, sizeof(kpeer));
    kpeer.nl_family = AF_NETLINK;
    kpeer.nl_pid = 0;
    kpeer.nl_groups = 0;

    memset(message, '\0', sizeof(struct nlmsghdr));
    message->nlmsg_len = NLMSG_SPACE(strlen(data));
    message->nlmsg_flags = 0;
    message->nlmsg_type = 0;
    message->nlmsg_seq = 0;
    message->nlmsg_pid = local.nl_pid;

    retval = memcpy(NLMSG_DATA(message), data, strlen(data));

    printf("message sendto kernel are:%s, len:%d\n", (char *)NLMSG_DATA(message), message->nlmsg_len);
    ret = sendto(skfd, message, message->nlmsg_len, 0,(struct sockaddr *)&kpeer, sizeof(kpeer));
    if(!ret){
        perror("send pid:");
        exit(-1);
    }

	while(cnt--){
    	//接受内核态确认信息
	    ret = recvfrom(skfd, &info, sizeof(struct u_packet_info),0, (struct sockaddr*)&kpeer, &kpeerlen);
    	if(!ret){
        	perror("recv form kerner:");
	   //     exit(-1);
    	}
    	printf("message receive from kernel:%s\n",(char *)info.msg);
	}

    //内核和用户进行通信

		sleep(3);
	free(message);
    close(skfd);
    return 0;
}
